...
void imprimir_resultados(double tiempo, int b, int m, int n) {
    double mflops; 
...
int main(int argc,char *argv[]) {
    ...
    if (argc == 5)              // si se han pasado valores en linea de comandos, se comprueban
    {
        m = atoi(argv[1]);
        n = atoi(argv[2]);
        k = atoi(argv[3]);
    ...
    // fin=omp_get_wtime();
    // imprimir_resultados(fin-start, b, m, n);
