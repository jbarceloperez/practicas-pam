...
void imprimir_resultados(double tiempo, int b, int n) {
    double mflops; 
...
void mulmatcuablosec(double *m1, double *m2, double *mr, int n, int b, int e) {
    int i,j,k;
    double *aux;
    ...
    for (i = 0; i < n; i += b)
    {
        for (j = 0; j < n; j +=b)
        {
            ...
            for (k = 0; k < e; k++) // suma las tres matrices producto de la multiplicacion de bloques correspondientes
            {
...
int main(int argc,char *argv[]) {
    ...
    if (argc == 3)              // si se han pasado valores en linea de comandos, se comprueban
    {
        n = atoi(argv[1]);
        b = atoi(argv[2]);
        if (n%b!=0)
    ...
    start=omp_get_wtime();
    mulmatcuablosec(m1, m2, mr, n, b, e);      //MULTIPLICACION
    // escribir(mr, n); // debug
    fin=omp_get_wtime();
    imprimir_resultados(fin-start, b, n);
